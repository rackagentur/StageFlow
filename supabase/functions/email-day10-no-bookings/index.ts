...
const emailTemplate = (name: string, repliedCount: number) => {
  const repliedPlural = repliedCount === 1 ? "lead" : "leads";
...
<head>
  ...
  <title>You're close — let's close the deal</title>
</head>
          ...
          <tr>
            <td style="padding: 0;">
              ...
              <h2 style="margin: 0 0 24px 0; font-size: 28px; font-weight: 700; line-height: 1.2; color: #ffffff;">
                You're close — let's close the deal
              </h2>
              ...
              <p style="margin: 0 0 20px 0; font-size: 16px; line-height: 1.6; color: #a1a1aa;">
                I see you've got ${repliedCount} ${repliedPlural} in the "Replied" stage.
              </p>
              
              <p style="margin: 0 0 20px 0; font-size: 16px; line-height: 1.6; color: #a1a1aa;">
                That means venues are interested.
              </p>
              
              <p style="margin: 0 0 20px 0; font-size: 16px; line-height: 1.6; color: #a1a1aa;">
                They responded. They're paying attention. Now it's time to turn that interest into a booking.
              </p>
              
              <p style="margin: 0 0 32px 0; font-size: 16px; line-height: 1.6; color: #a1a1aa;">
                Here's what I learned after years of hustling: <strong style="color: #ffffff;">the follow-up is everything</strong>.
              </p>
                  ...
                  <td style="padding: 24px;">
                    ...
                    <ul style="margin: 0; padding: 0 0 0 20px; color: #a1a1aa;">
                      <li style="margin: 0 0 12px 0; line-height: 1.6;">
                        <strong style="color: #ffffff;">Propose specific dates.</strong> Don't ask "when works for you?" — offer 2-3 dates.
                      </li>
                      <li style="margin: 0 0 12px 0; line-height: 1.6;">
                        <strong style="color: #ffffff;">Make it easy.</strong> Send your rider, your tech specs, your rate. Remove every excuse.
                      </li>
                      <li style="margin: 0; line-height: 1.6;">
                        <strong style="color: #ffffff;">Follow up again.</strong> If they don't respond in 3 days, send one more message. Polite. Direct. Move forward.
                      </li>
                    </ul>
                  </td>
                </tr>
...
serve(async (req) => {
  ...
  try {
    ...
    const { userId, email, name, repliedCount }: UserData = await req.json();
    ...
    const { data: alreadySent } = await supabase
      ...
      .select("id")
      .eq("user_id", userId)
      .eq("email_type", "day10_no_bookings")
      .single();
    ...
    const resendResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: "You're close — let's close the deal",
        html: emailTemplate(name, repliedCount),
      }),
    });
    ...
    await supabase.from("email_sends").insert({
      user_id: userId,
      email_type: "day10_no_bookings",
      resend_id: resendData.id,
      metadata: { email, name, repliedCount },
    });
